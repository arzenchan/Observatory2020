...
def import_file_xml(xml_file_name, output_location, geo_ident):
...
#--------DEPRECIATED CODE BEGIN--------
def to_geojson2(df, properties, output_location, lat='lat', lon='long'):
    ...
    output_filename = (output_location + ".geojson")
    with open(output_filename, 'w') as output_file:
        json.dump(geojson, output_file, indent=2) 
    
    return geojson

def to_geojson3(df, output_location):
    """
    This function to export to GeoJSON taken from 
    https://gist.github.com/mapcloud/036688121a87f960e35cc03077001e93
    which is based upon
    https://gis.stackexchange.com/questions/220997/pandas-to-geojson-multiples-points-features-with-python?rq=1
    """
    ...
    with open(output_location+'.geojson', 'w') as fp:
...
def geo_code(raw_data, geo_ident):
    ...
    test_sample = True #make false to run whole dataset
    if test_sample:
...
def get_address_data(address):#using OSM Nominatim API
    ...
    if resp == '[]':
        ...
        geo_data[1] = -73.532947 #"Not Found"
        geo_data[2] = 45.432397,  #"Not Found"
...
def main():
    ...
    xml_file_name = "C:/Users/Arzen/OneDrive - McGill University/2020 Work/Food Inspection Offenders/cusersuarcherdesktopinspection-aliments-contrevenants_SHORT.xml.xml"
    ...
    geolocated_data = import_file_xml(xml_file_name, output_file, address_column)
    data_out(geolocated_data, output_location)
